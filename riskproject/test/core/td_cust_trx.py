...
def get_trx_detail(parm_csv_reader):
    rst = pd.DataFrame({})
    print('parm_csv_reader = ',parm_csv_reader.line_num)
...
def get_cust_trx_hist(parm_csv_floder, parm_csv_file_list):
    ...
    for csv_file in parm_csv_file_list:
        ...
        df_trx_detail = get_trx_detail(csv_reader)
        print('df_trx_detail =', df_trx_detail.iloc[:, 0].size)
        # print('df_trx_detail \n', df_trx_detail)
...
def get_cust_trx_hist_bak(csv_reader):
    print(os.path.basename(__file__), sys._getframe().f_code.co_name, sys._getframe().f_lineno)
    rtn_df = pd.DataFrame({})
