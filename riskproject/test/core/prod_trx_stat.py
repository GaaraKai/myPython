...
def overall_rpt(parm_rst):
    ...
    logger.info("[2]. STATISTICS RANGE: FROM %s TO %s" %
                (str(parm_rst.head(1)["plat_date"].values)[2:12],
                 str(parm_rst.tail(1)["plat_date"].values)[2:12]))

    logger.info("[3]. TOTAL TRANSACTION COUNT: %s" % CAP_TRX_CNT)

    logger.info("[4]. TOTAL TRANSACTION AMOUNT: %s BILLION" % round(TOT_TRX_AMT / 1000000000, 2))

    logger.info("[5]. MERCHANT STATISTIC:")
    stat_of_top("mer_id", parm_rst)

    logger.info("[6]. PRODUCT STATISTIC:")
...
def data_preproc(parm_csv_folder, parm_csv_file_list):
    ...
    for csv_file in parm_csv_file_list:
        ...
        csv_file_path = "D:/github_program/myPython/docs/csvfiles/prod_trx_stat/NO 1_ trx_dtl"
        # csv_file_path = "D:/github_program/myPython/docs/csvfiles/todo_td/NO 1_td_1"
...
def stat_of_top(parm_dimen, parm_rst):
    ...
    logger.info("==>> TOTAL MERCHANT COUNT: %s " % tot_cnt)
    logger.info("==>> TOP10 MERCHANT:")
    rst = pd.pivot_table(parm_rst, index=[parm_dimen],aggfunc=np.sum).reset_index()
    logger.info("==>> FAILED: %s " % get_top(rst, parm_dimen, "failed_cnt"))
    logger.info("==>> INTERCEPT: %s " % get_top(rst, parm_dimen, "intercept_cnt"))
    logger.info("==>> SUCCESS: %s " % get_top(rst, parm_dimen, "success_cnt"))


def stat_of_prod(parm_rst):
    pass
...
def create_rpt(parm_rst):
    if len(parm_rst) != 0:
        ...
        # stat_of_mer(parm_rst)
        logger.info("--------------------------------------------")
        # 3. STATISTICS OF PRODUCT
        # stat_of_prod(parm_rst)
